...
%	Output: 
%		1 - [column vector 4x1]   data pack w/o header
%
...
function [dati] = Data_Pack_quaternion(u,comPort,bracialet)
...
a=true;
    ...
    if isempty(s)
        ...
        if u==1
            ...
            txdata = ['01';'92';'FD';'09';'00';'00';bracialetByte;'00';'4F';'10';'00';'00';'00']; %13
    ...
    %-------------------------------------------------------------%
    %--- read data pack (EMG HEADER [13] + EMG DATA [4*float]) ---%
    %-------------------------------------------------------------% 
    if  scambio_dati 
        dati0 = zeros(13,1);
        dati0 = fread(s,13); 
        dati = zeros(4,1);
        dati = fread(s,4,'float'); % w x y z
